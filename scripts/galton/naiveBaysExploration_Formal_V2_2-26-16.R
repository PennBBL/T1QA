...
install_load('corrplot', 'ggplot2', 'psych', 'e1071', 'pROC', 'ggm', 'plyr', 'visreg', 'scales')
...
residualizeInputRow <- function(dataFrame, colToResid, qaMetricVal){
   ...
   regressedValues <- lm(dataFrame[,colToResid]~dataFrame[,grepQA]+ageCol#+ageSquared+ageCubed
                                               +sexCol+raceCol#+icvCol
                                               , data=dataFrame, 
                                               na.action=na.omit)
   return(regressedValues)
} 

residualizeInputRowNoQA <- function(dataFrame, colToResid){
   raceCol <- dataFrame$race2
   sexCol <- dataFrame$sex
   ageCol <- dataFrame$ageAtGo1Scan
   icvCol <- dataFrame$mprageMassICV
   ageSquared <- ageCol^2
   ageCubed <- ageCol^3   
   regressedValues <- lm(dataFrame[,colToResid]~ageCol#+ageSquared+ageCubed
                                               +sexCol+raceCol#+icvCol
                                               , data=dataFrame, 
                                               na.action=na.omit)
   return(regressedValues)
...
# Stellar
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==2)]/12, main="Distribution of 2 Subject Age", xlab="Subject Age in Years")

# Good
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==1.667)]/12, main="Distribution of 1.667 Subject Age", xlab="Subject Age in Years")

# Decent
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==1)]/12, main="Distribution of 1 Subject Age", xlab="Subject Age in Years")

# Bad
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==0)]/12, main="Distribution of 0 Subject Age", xlab="Subject Age in Years")


# Now perform a t test between the two population age's
#t.test(mergedQAP$ageAtGo1Scan, formula = mergedQAP$ageAtGo1Scan ~ mergedQAP[,manualQAColVal])
...
# Now Stellar Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==2)]), main = "n of Gender for 2 Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))

# Now Good Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==1.667)]), main = "n of Gender for 1.667 Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))

# Now Good Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==1)]), main = "n of Gender for 1 Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))

# Now Non-Flagged Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==0)]), main = "n of Gender for 0 Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))
...
## Now do all of the previous graphs in one.... using visreg
mergedQAP$sex <- as.factor(mergedQAP$sex)
mergedQAP$sex <- revalue(mergedQAP$sex, c('1'='Male', '2'='Female'))
mergedQAP$averageRating <- as.factor(mergedQAP$averageRating)
fit <- lm(ageAtGo1Scan/12 ~ averageRating + sex, data=mergedQAP)
visreg(fit, "averageRating", by="sex", overlay = TRUE, xlab="Rating Bin", ylab="Age")
...
for(qapMetricVal in 1:length(qapValNames)){
  ...
  for(neuroImgVal in grepPattern[1]){
    ...
    for(grepPatternCol in allGrepPatternCols){
      fittedPVal <- summary(residualizeInputRow(mergedQAP, grepPatternCol, qapMetricVal))$coefficients[3,4]
...
dataFrameToExportNoQA <- data.frame()
for(qapMetricVal in 1){
  ## Change the value in grepPattern to work with differnt neuroimaging values ##
  for(neuroImgVal in grepPattern[1]){
    allGrepPatternCols <- grep(neuroImgVal, names(mergedQAP))
    vectorToBind <- vector()
    allGrepPatternCols <- append(allGrepPatternCols, 145)
    for(grepPatternCol in allGrepPatternCols){
      fittedPVal <- summary(residualizeInputRowNoQA(mergedQAP, grepPatternCol))$coefficients[2,4]
      vectorToBind <- append(vectorToBind, fittedPVal)
    }
    dataFrameToExportNoQA <- rbind(dataFrameToExportNoQA, vectorToBind)
  }
}
colnames(dataFrameToExportNoQA) <- names(mergedQAP)[allGrepPatternCols]
rownames(dataFrameToExportNoQA) <- 'No QAP'
...
for(qapMetricName in 1){
  for(neuroImgVal in grepPattern[1]){
    allGrepPatternCols <- grep(neuroImgVal, names(mergedQAP)) 
    allGrepPatternCols <- append(allGrepPatternCols, 145)
    p.cor.vector <- vector()
    for(grepPatternCol in 145){
      p.cor.string <- c(names(mergedQAP)[grepPatternCol], "ageAtGo1Scan", "sex", "race2")
      tmp <- mergedQAP[complete.cases(mergedQAP[,grepPatternCol]),]
      #tmp <- tmp[complete.cases(tmp$mprageMassICV),]
      p.cor.val <- pcor(p.cor.string, var(tmp[p.cor.string])) 
      p.cor.vector <- append(p.cor.vector, p.cor.val)      
...
colnames(p.cor.data.frame) <- names(mergedQAP)[145]
rownames(p.cor.data.frame) <- c(qapValNames, 'No QAP')
...
barPlotDataFrame <- rbind(barPlotDataFrame, apply(dataFrameToExportNoQA,2,function(x){ifelse(x<0.05,1,0)}))
rownames(barPlotDataFrame)[37] <- 'No QAP' 
...
bar.plot.p.cor.data.frame$qapVals <- as.factor(rownames(p.cor.data.frame))
...
ggplot(barPlotDataFrame, aes(x=qapVals,y=as.numeric(as.character(sigVal)), fill=as.numeric(as.character(sigVal)))) + 
      ...
      labs(title="# Sig ROIS Age Sig Predic's FSCT", x="QAP Measures", y="# of Sig  Values") + 
      scale_y_continuous(breaks=seq(60,69)) + 
      theme(legend.position='none') + 
      scale_y_continuous(limits=c(62,69),oob=rescale_none)
...
for(qapMetricVal in qapValNames){
  ...
  model <- naiveBayes(averageRating ~ isolatedVars[,qapCol], data=isolatedVars)
...
for(qapMetricVal in qapValNames){
  ...
  model <- glm(averageRating~isolatedVars[,qapCol], data=isolatedVars, family="binomial")
  pVal <- summary(model)$coefficients[2,4]
  predicVals <- predict(model, newdata=isolatedVars, type='response')
  areaUnderCurve <- auc(roc(response=isolatedVars$averageRating, predictor=predicVals, levels=c(0,1), auc=TRUE,    ci=TRUE, plot.roc=TRUE))
...
ggplot(dataFrameToPlot, aes(x=V1,y=as.numeric(as.character(areaUnderCurveMulti)))) + 
...
###################################
###################################
########GO1 Manual Rating Fin######
#######Work Starts Down Here#######
#############2/22/16###############
###################################

## Prepare a data frame with just the Good vs Bad images
isolatedVars <- mergedQAP[qapValNames]
isolatedVars <- cbind(isolatedVars, mergedQAP[manualQAValue])
isolatedVars[manualQAValue] <- as.factor(isolatedVars$averageRating)
...
for(qapMetricVal in qapValNames){
  ...
  model <- glm(averageRating~train[,qapCol], data=train, family=binomial())
  pVal <- summary(model)$coefficients[2,4]
  predicVals <- predict(model, newdata=goodvsBadData, type='response')
  areaUnderCurve <- auc(roc(response=goodvsBadData$averageRating, predictor=predicVals, levels=c(0,1), auc=TRUE,    ci=TRUE, plot.roc=TRUE))
  vectorToAppend <- cbind(qapMetricVal, areaUnderCurve)
  vectorToAppendTwo <- cbind(qapMetricVal, pVal)
  dataFrameToPlot <- rbind(dataFrameToPlot, vectorToAppend)
  pValDataFrame <- rbind(pValDataFrame, vectorToAppendTwo)
...
dataFrameToPlot$areaUnderCurve <- as.numeric(as.character(dataFrameToPlot$areaUnderCurve))
ggplot(dataFrameToPlot, aes(x=as.factor(qapMetricVal),y=areaUnderCurve, fill=areaUnderCurve)) + geom_bar(stat="identity", width=0.4, position=position_dodge(width=0.5)) + theme(axis.text.x = element_text(angle=45,hjust=1)) + labs(title="Area Under ROC Curve Predicting mprageSbiaExclude", x="QAP Measures", y="Area Under ROC Curve") + coord_cartesian(ylim=c(.4,.95))

listOfSigVals <- as.character(pValDataFrame[which(as.numeric(as.character(pValDataFrame[,2]))<.01),1])
listOfRegressors <- c("sex","race2","ageAtGo1Scan")
multiFormula <- as.formula(paste(manualQAValue, "~", paste(listOfSigVals,collapse="+")))
model <- glm(multiFormula, data=train, family=binomial())
predicVals <- predict(model, newdata=goodvsBadData, type='response')
areaUnderCurve <- auc(roc(response=goodvsBadData$averageRating, predictor=predicVals, levels=c(0,1), auc=TRUE,    ci=TRUE, plot.roc=TRUE))
...
for(qapMetricVal in qapValNames){
  qapCol <- grep(qapMetricVal, names(goodvsBadData))
  if(length(qapCol) > 1){
    qapCol <- qapCol[1]
  }
  model <- naiveBayes(averageRating ~ goodvsBadData[,qapCol], data=goodvsBadData)
  flagsPredic <- unname(table(predict(model,newdata=goodvsBadData)))[2]
  vectorToAppend <- cbind(qapMetricVal, flagsPredic)
  naiveBaysPredicted <- rbind(naiveBaysPredicted, vectorToAppend)
} 
...
data <- goodvsBadData 
train <- data
train[,37] <- as.character(train[,37])
enet.alphas<-seq(0,1,by=0.01)
enet.optparam<-matrix(nrow=101,ncol=3)
colnames(enet.optparam)<-c("Alpha","Lambda","CVM")
count=1
C=length(train)-1

for (a in enet.alphas){
	enet.alphas.cv<-cv.glmnet(as.matrix(train[,1:36]),train[,37],alpha=a, standardize=FALSE,nfolds=10,family="binomial")
...
mod <- glmnet(as.matrix(train[,1:36]), train[,37], standardize=FALSE, alpha=optval[1],lambda=optval[2],family="binomial")

mod$beta

allFormula <- as.formula(paste(manualQAValue, "~", paste(qapValNames,collapse="+")))
train <- goodvsBadData[1:200,]
train[,37] <- as.character(train[,37])
model <- naiveBayes(allFormula, data=train)
