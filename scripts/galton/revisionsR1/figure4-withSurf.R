...
for(qapVal in varsOfInterest){
  all.train.data[qapVal] <- scale(all.train.data[qapVal], center=T, scale=T)
  valsToAppend <- summarySE(data=all.train.data, measurevar=qapVal, groupvars='averageRating.y', na.rm=T)
...
i <- 1
for(qapVal in varsOfInterest){
  all.valid.data[qapVal] <- scale(all.valid.data[qapVal], center=T, scale=T)
  valsToAppend <- summarySE(data=all.valid.data, measurevar=qapVal, groupvars='averageRating.y', na.rm=T)
  qapValue <- rep(prettyNames[i], nrow(valsToAppend))
...
corVals <- NULL
i <- 1
residAverageRating <- residuals(lm(averageRating.y ~ ageAtGo1Scan + s + ageAtGo1Scan^2, data=all.train.data))
for(qapVal in varsOfInterest){
  paste(qapVal)
  sel <- which(!is.na(all.train.data[qapVal]))
  form1 <- as.formula(paste(qapVal,' ~ ageAtGo1Scan + s + ageAtGo1Scan^2', sep=''))
  residQuant <- residuals(lm(form1, data=all.train.data))
  corVal <- cor(residAverageRating[sel], residQuant, method='spearman')
  corPVal <- cor.test(residAverageRating[sel], residQuant, method='spearman')$p.value
...
for(qapVal in varsOfInterest){
  paste(qapVal)
  sel <- which(!is.na(all.valid.data[qapVal]))
  form1 <- as.formula(paste(qapVal,' ~ ageAtGo1Scan + s + ageAtGo1Scan^2', sep=''))
