...
trainingData$zeroVsNotZeroOutcome <- predict(zeroVsNotZeroModel, newdata=trainingData, allow.new.levels=T, type='response')
validationData$oneVsTwoOutcome <- predict(oneVsTwoModel, newdata=validationData, allow.new.levels=T, type='response')
validationData$zeroVsNotZeroOutcome <- predict(zeroVsNotZeroModel, newdata=validationData, allow.new.levels=T, type='response')
...
bg2 <- ggplot(bg2.vals[which(bg2.vals$Dataset=='Validation'),], aes(x=factor(sex), y=as.numeric(as.character(oneVsTwoOutcome)), group=Dataset)) +
		...
		axis.ticks.y=element_blank(),
                ...
                title=element_text(size=30)) + 
		geom_path(aes(x=factor(sex), y=c(.95,.95))) +
		geom_path(aes(x=factor(sex)[1], y=c(.95, .89))) +
		geom_path(aes(x=factor(sex)[2], y=c(.94, .95))) +
		geom_text(aes(x=factor(sex)[1], y=.97), label='***',angle=90, size=10) +
		scale_y_continuous(limits=c(0, 1),
                           breaks=round(seq(.8, 1, .2), digits=2), oob=rescale_none) +
...
## Now do the same for the 0vs!0 model
# Now prepare our values
bg2.vals.train <- summarySE(data=all.train.data, measurevar='zeroVsNotZeroOutcome', groupvars='sex')
bg2.vals.train$Dataset <- rep('Training', nrow(bg2.vals.train))
bg2.vals.valid <- summarySE(data=all.valid.data, measurevar='zeroVsNotZeroOutcome', groupvars='sex')
bg2.vals.valid$Dataset <- rep('Validation', nrow(bg2.vals.valid))
bg2.vals <- rbind(bg2.vals.train, bg2.vals.valid)
bg2.vals$Dataset <- factor(bg2.vals$Dataset)
bg2.vals$sex <- c('Male', 'Female', 'Male', 'Female')
...
# Now lets plot our values
# Grab a p value from a t.test
pValue <- t.test(all.train.data$zeroVsNotZeroOutcome ~ all.train.data$sex)
pValue <- wilcox.test(all.train.data$zeroVsNotZeroOutcome ~ all.train.data$sex)
bg1a <- ggplot(bg2.vals[which(bg2.vals$Dataset=='Training'),], aes(x=factor(sex), y=as.numeric(as.character(zeroVsNotZeroOutcome)), group=Dataset)) +
geom_bar(stat='identity', position=position_dodge(), width=.5) +
labs(title='Training', x='Sex', y='Manual Quality Rating (mean value)') +
theme_bw() +
coord_cartesian(ylim=c(.9,1.1)) +
geom_errorbar(aes(ymin=as.numeric(as.character(zeroVsNotZeroOutcome))-se,
ymax=as.numeric(as.character(zeroVsNotZeroOutcome))+se),
width = .1, position=position_dodge(.9)) +
#facet_grid(Dataset ~ .) +
theme(legend.position="none",
axis.text=element_text(size=20),
...
strip.text.y = element_text(size = 16, angle = 270, face="bold"),
title=element_text(size=30)) +
geom_path(aes(x=factor(sex), y=c(1.05,1.05))) +
geom_path(aes(x=factor(sex)[1], y=c(1.05, .99))) +
geom_path(aes(x=factor(sex)[2], y=c(1, 1.05))) +
geom_text(aes(x=factor(sex)[1], y=1.07), label='***',angle=90, size=10) +
scale_y_continuous(limits=c(0, 1),
breaks=round(seq(.8, 1, .2), digits=2), oob=rescale_none) +
annotate("text", x=c(Inf), y=c(Inf), label="p < 0.001", hjust=c(2.1), vjust=c(1.1), size=8, parse=T)


pValue <- t.test(all.valid.data$zeroVsNotZeroOutcome ~ all.valid.data$sex)
pValue <- wilcox.test(all.valid.data$zeroVsNotZeroOutcome ~ all.valid.data$sex)
bg2a <- ggplot(bg2.vals[which(bg2.vals$Dataset=='Validation'),], aes(x=factor(sex), y=as.numeric(as.character(zeroVsNotZeroOutcome)), group=Dataset)) +
geom_bar(stat='identity', position=position_dodge(), width=.5) +
labs(title='Validation', x='Sex', y='Manual Quality Rating (mean value)') +
theme_bw() +
coord_cartesian(ylim=c(.9,1.1)) +
...
# Now build our models to show geneerl age trends
corVal <- cor(all.train.data$zeroVsNotZeroOutcome, all.train.data$age, method='spearman')
corSig <- cor.test(all.train.data$zeroVsNotZeroOutcome, all.train.data$age, method='spearman')$p.value
corText1 <- expression("Spearman's"~rho == .15)
corText2 <- paste("p < 0.01")
mod1a <- ggplot(all.train.data, aes(x=zeroVsNotZeroOutcome, y=age)) +
geom_smooth(method=lm, color='black') +
theme_bw() +
...
corVal <- cor(all.valid.data$zeroVsNotZeroOutcome, all.valid.data$age, method='spearman')
corSig <- cor.test(all.valid.data$zeroVsNotZeroOutcome, all.valid.data$age, method='spearman')$p.value
corText1 <- expression("Spearman's"~rho == paste(0.31))
corText2 <- paste("p < 0.01")
mod2a <- ggplot(all.valid.data, aes(x=zeroVsNotZeroOutcome, y=age)) +
geom_smooth(method=lm, color='black') +
theme_bw() +
coord_cartesian(ylim=c(5,20)) +
labs(title='', x='Manual Quality Rating (mean value)', y='Age') +
...
png('figure3-demographicsvs0vsNot0QAPPaper.png', width=16, height=16, units='in', res=300)
multiplot(bg1a, mod1a, bg2a, mod2a, cols=2)
dev.off()
png('figure3-allValues.png', width=16, height=32, units='in', res=300)
multiplot(bg1, mod1, bg2, mod2, bg1a, mod1a, bg2a, mod2a, cols=2)
dev.off()
