...
all.train.data <- merge(all.train.data, eulerNumber, by='bblid')
all.valid.data <- merge(validationData, manualQAData, by='bblid')
all.valid.data <- merge(all.valid.data, eulerNumber, by='bblid')

# Now prepare our values
bg2.vals.train <- summarySE(data=all.train.data, measurevar='mean_euler', groupvars='sex')
bg2.vals.train$Dataset <- rep('Training', nrow(bg2.vals.train))
bg2.vals.valid <- summarySE(data=all.valid.data, measurevar='mean_euler', groupvars='sex')
...
pValue <- t.test(all.train.data$mean_euler ~ all.train.data$sex)
...
bg1 <- ggplot(bg2.vals[which(bg2.vals$Dataset=='Training'),], aes(x=factor(sex), y=as.numeric(as.character(mean_euler)), group=Dataset)) +
                ...
                labs(title='Training', x='Sex', y='Euler Numbere\n(mean value)') +
                theme_bw() + 
...
bg2 <- ggplot(bg2.vals[which(bg2.vals$Dataset=='Validation'),], aes(x=factor(sex), y=as.numeric(as.character(mean_euler)), group=Dataset)) +
                geom_bar(stat='identity', position=position_dodge(), width=.5) + 
...
corSig <- cor.test(all.train.data$oneVsTwoOutcome, all.train.data$age, method='spearman', use='complete')$p.value
corText1 <- expression("Spearman's"~rho == .42)
corText2 <- paste("p < 0.001")
mod1 <- ggplot(all.train.data, aes(x=mean_euler, y=age)) +
...
corVal <- cor(all.valid.data$mean_euler, all.valid.data$age, method='spearman')
corSig <- cor.test(all.valid.data$mean_euler, all.valid.data$age, method='spearman')$p.value
corText1 <- expression("Spearman's"~rho == paste(0.46))
corText2 <- paste("p < 0.001")
