...
# Stellar
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==2)]/12, main="Distribution of Flagged Subject Age", xlab="Subject Age in Years")

# Good
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==1.667)]/12, main="Distribution of Flagged Subject Age", xlab="Subject Age in Years")
...
# Bad
hist(mergedQAP$ageAtGo1Scan[which(mergedQAP[,manualQAColVal]==0)]/12, main="Distribution of Flagged Subject Age", xlab="Subject Age in Years")
...
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==2)]), main = "n of Gender for Stellar Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))

# Now Good Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==1.667)]), main = "n of Gender for Good Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))

# Now Good Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==1)]), main = "n of Gender for Decent Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))

# Now Non-Flagged Images Gender
barplot(table(mergedQAP$sex[which(mergedQAP[,manualQAColVal]==0)]), main = "n of Gender for Bad Images", xlab = "Gender", ylab = "n of Gender", names.arg=c("male", "female"))
...
dataFrameToPlot$areaUnderCurve <- as.numeric(as.character(dataFrameToPlot$areaUnderCurve))
ggplot(dataFrameToPlot, aes(x=as.factor(qapMetricVal),y=areaUnderCurve, fill=areaUnderCurve)) + geom_bar(stat="identity", width=0.4, position=position_dodge(width=0.5)) + theme(axis.text.x = element_text(angle=45,hjust=1)) + labs(title="Area Under ROC Curve Predicting mprageSbiaExclude", x="QAP Measures", y="Area Under ROC Curve") + coord_cartesian(ylim=c(.4,.85))
...
model <- glm(multiFormula, data=isolatedVars, family=binomial())
predicVals <- predict(model, newdata=isolatedVars, type='response')
areaUnderCurve <- auc(roc(response=isolatedVars$averageRating, predictor=predicVals, levels=c(0,1), auc=TRUE,    ci=TRUE, plot.roc=TRUE))
