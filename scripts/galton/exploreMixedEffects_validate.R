...
plot(roc(as.vector(predict(lmm.1, type='response')), lme.data$binaryOutcome), main='glmer PCA Pitt Validation')
legend(x='bottomright', legend=paste('AUC = ', auc(roc(as.vector(predict(lmm.1, type='response')), lme.data$binaryOutcome))))
