...
for(i in motionCols){
  roc.tmp <- roc(raw.lme.data.test$averageRating.x ~ raw.lme.data.test[,i])
  auc.tmp <- auc(roc.tmp)
  outTmp <- c(names(raw.lme.data)[i], auc.tmp, '0 vs !0', 'Testing')
  aucVals <- rbind(aucVals, outTmp)
}
...
for(i in motionCols){
  roc.tmp <- roc(raw.lme.data.test$averageRating ~ raw.lme.data.test[,i])
  auc.tmp <- auc(roc.tmp)
  outTmp <- c(names(raw.lme.data)[i], auc.tmp, '1 vs 2', 'Testing')
  aucVals <- rbind(aucVals, outTmp)
}
...
# Now plot our data
png('figure10-aucCompare.png', height=8, width=8, units='in', res=300)
aucPlot
dev.off()
