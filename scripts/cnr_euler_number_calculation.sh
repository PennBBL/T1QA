...
#Previous BBL scripts to calculate cnr only pulled the total cnr, this script pulls total, gray/csf for left and right hemispheres and gray/white for both hemispheres in addition to calculating the euler number (tells how many holes/defects are in the surface) as is done in the methods of the Chalavi, et al paper

#set freesurfer specific variables (unique to GO1/GO2 project on cfn)
